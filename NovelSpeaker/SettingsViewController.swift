...
class SettingsViewController: FormViewController, MFMailComposeViewControllerDelegate, RealmObserverResetDelegate, AppInformationAliveDelegate, UIDocumentBrowserViewControllerDelegate, UIDocumentPickerDelegate {
    ...
    func createSettingsTable(){
        ...
        <<< MultipleSelectorRow<String>("menuItemsNotRemovedRow") { row in
            ...
            section
            <<< SwitchRow() {
                $0.title = NSLocalizedString("SettingTableViewController_IsSkipForewordAfterwordOnNewImport", comment: "新しく取り込む章の前書き・後書きを読み上げない(取込設定が未設定のサイト/小説にのみ有効)")
                $0.cell.textLabel?.numberOfLines = 0
                $0.value = RealmUtil.RealmBlock { (realm) -> Bool in
                    return RealmGlobalState.GetInstanceWith(realm: realm)?.isSkipForewordAfterwordOnNewImport ?? false
                }
            }.onChange({ row in
                guard let value = row.value else { return }
                RealmUtil.RealmBlock { (realm) -> Void in
                    guard let globalState = RealmGlobalState.GetInstanceWith(realm: realm) else { return }
                    RealmUtil.WriteWith(realm: realm) { (realm) in
                        globalState.isSkipForewordAfterwordOnNewImport = value
                        realm.add(globalState, update: .modified)
                    }
                }
            })
