...
struct StorySiteInfo : Identifiable {
    ...
    // pageElementDict の key、またはその表示タイトル(前書き/後書き/foreword/afterword等)から、
    // その要素が「前書き」または「後書き」を表す物かどうかを推測します。
    // isSkipForewordAfterwordOnNewImport が ON の時に、取込設定が未設定のサイト/小説について
    // デフォルトの取込対象からこれらを除外するために使います。
    static let forewordAfterwordKeywords: [String] = ["foreword", "afterword", "前書き", "まえがき", "後書き", "あとがき"]
    func isForewordOrAfterwordElement(key: String) -> Bool {
        let lowerKey = key.lowercased()
        for keyword in StorySiteInfo.forewordAfterwordKeywords {
            if lowerKey.contains(keyword.lowercased()) { return true }
        }
        guard let titles = pageElementDict[key]?.0 else { return false }
    ...
    func resolveNovelImportEnableSettings(siteInfo: StorySiteInfo, novelID: String?) -> [String]? {
        ...
        return RealmUtil.RealmBlock { realm -> [String]? in
            ...
            guard let setting = setting else {
                // ユーザによる明示的な取込設定(サイト単位/小説単位どちらも)が無い場合のデフォルト解決。
                // グローバル設定がONなら、前書き・後書きらしき要素をデフォルトの取込対象から外します。
                // (既に取込設定を保存済みのサイト/小説には影響しません。)
                if RealmGlobalState.GetInstanceWith(realm: realm)?.isSkipForewordAfterwordOnNewImport == true {
                    let currentIds = Set(siteInfo.pageElementDict.keys)
                    let filteredIds = currentIds.filter { !siteInfo.isForewordOrAfterwordElement(key: $0) }
                    // 前書き・後書き"以外"の要素が無い(全部前書き/後書き扱いになってしまう)場合は
                    // 除外すると本文が空になってしまうので、フォールバックとして全件を対象にします。
                    if !filteredIds.isEmpty && filteredIds.count < currentIds.count {
                        return filteredIds.sorted()
                    }
                }
                return nil
            }
