    ...
    init(site: StorySiteInfo, scopeType: RealmNovelImportSetting.ScopeType = .site, novelID: String? = nil, setting: RealmNovelImportSetting?) {
        ...
        if let managedSetting = setting {
            ...
            let initialSetting = RealmUtil.WriteReturn { realm -> RealmNovelImportSetting in
                ...
                let allKeys = Set(site.pageElementDict.keys)
                var defaultSelectedKeys = allKeys
                if RealmGlobalState.GetInstanceWith(realm: realm)?.isSkipForewordAfterwordOnNewImport == true {
                    let filteredKeys = allKeys.filter { !site.isForewordOrAfterwordElement(key: $0) }
                    if !filteredKeys.isEmpty && filteredKeys.count < allKeys.count {
                        defaultSelectedKeys = filteredKeys
                    }
                }
                initialSetting.targets.append(objectsIn: defaultSelectedKeys.sorted())
