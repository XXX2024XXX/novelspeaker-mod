...
@objc class RealmUtil : NSObject {
    ...
    // 19 -> 20: RealmGlobalState に isSkipForewordAfterwordOnNewImport を追加した。
    //            Bool の追加時の既定値(false)と一致するため、メンバの「追加」だけで
    //            データ変換は不要(MigrateFunc への追記も不要)だが、
    //            schemaVersion を上げないと既存の Realm が開けなくなる。
...
@objc final class RealmGlobalState: Object {
    ...
    // ON にすると、取込設定(RealmNovelImportSetting)が未設定のサイト/小説について、
    // 新規にダウンロードする章から「前書き」「後書き」らしき要素をデフォルトで取り込み対象から外します。
    // (StorySiteInfo.isForewordOrAfterwordElement() で判定。既に取込設定を個別に保存済みのサイト/小説には影響しません。)
    @objc dynamic var isSkipForewordAfterwordOnNewImport = false
...
@objc final class RealmNovelTag: Object {
    ...
    func targetNovelArrayWith(realm:Realm) -> [RealmNovel]? {
        let novels = realm.objects(RealmNovel.self)
        var novelID2NovelMap = [String:RealmNovel]()
        for novel in novels {
            novelID2NovelMap[novel.novelID] = novel
        }
        return targetNovelArrayFrom(novelID2NovelMap: novelID2NovelMap)
    }
